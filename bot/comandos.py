...
async def cmd_horarios(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.effective_chat.type in ["group", "supergroup"]:
        await context.bot.send_message(update.effective_user.id, texto, parse_mode=ParseMode.MARKDOWN)
    else:
        await update.message.reply_text(texto, parse_mode=ParseMode.MARKDOWN)
...
async def cmd_aniversario(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("❌ Formato inválido. Use: /aniversario DD/MM\n_Ex: /aniversario 25/12_", parse_mode=ParseMode.MARKDOWN)
        return
    registrar_aniversario(user_id, nome, dia, mes)
    total = total_cadastrados()
    await update.message.reply_text(
        f"🎂 *Aniversário cadastrado!*\n\n📅 Data: *{dia:02d}/{mes:02d}*\n👤 Nome: *{nome}*\n\n_Já são {total} membros cadastrados!_ 🙏",
        parse_mode=ParseMode.MARKDOWN
    )
...
async def cmd_oracao(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if context.args:
        ...
        try:
            ...
            pass
        await update.message.reply_text(
            f"🙏 *Pedido de oração registrado!*\n\n_{pedido}_\n\nOs irmãos vão interceder por você!\n_\"Confessai as vossas ofensas uns aos outros...\"_ — Tiago 5:16",
            parse_mode=ParseMode.MARKDOWN
        )
        ...
        if not pedidos:
            await update.message.reply_text(
                "🙏 *Nenhum pedido de oração pendente.*\n\nPara fazer um pedido use:\n`/oracao [seu pedido]`",
                parse_mode=ParseMode.MARKDOWN
            )
            return
...
async def cmd_postar_versiculo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await cmd_postar(update, context)
...
async def cmd_fila(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    texto = (
        ...
        f"📅 Postagem automática: *09h e 19h*\n\n_Envie vídeos/fotos no privado do bot para adicionar._"
    )
    destino = update.effective_user.id if update.effective_chat.type in ["group", "supergroup"] else update.effective_chat.id
    await context.bot.send_message(destino, texto, parse_mode=ParseMode.MARKDOWN)
...
async def cmd_perdoar(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(
        update.effective_chat.id,
        f"✅ Os avisos de *{alvo.first_name}* foram zerados.\n\n_\"Perdoando-vos uns aos outros.\"_ — Efésios 4:32",
        parse_mode=ParseMode.MARKDOWN
    )
...
async def cmd_info(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        texto = (
            f"👤 *INFORMAÇÕES DO MEMBRO*\n\nNome: *{alvo.first_name} {alvo.last_name or ''}*\n"
            f"Username: @{alvo.username or 'sem username'}\nID: `{alvo.id}`\n"
            f"Status: *{status}*\n⚠️ Avisos: *{total_avisos}/3*\n🤖 Bot: {'Sim' if alvo.is_bot else 'Não'}"
        )
        destino = update.effective_user.id if update.effective_chat.type in ["group", "supergroup"] else update.effective_chat.id
        await context.bot.send_message(destino, texto, parse_mode=ParseMode.MARKDOWN)
    except Exception as e:
...
async def cmd_listanegra(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not palavras:
        await context.bot.send_message(
            destino,
            f"🚫 *LISTA NEGRA*\n\nPalavrões padrão bloqueados: *{len(PALAVROES_EXATOS)}*\nPalavras personalizadas: *0*\n\n_Use /bloquear [palavra] para adicionar._",
            parse_mode=ParseMode.MARKDOWN
        )
    else:
        ...
        await context.bot.send_message(
            destino,
            f"🚫 *LISTA NEGRA*\n\nPalavrões padrão: *{len(PALAVROES_EXATOS)}*\nPalavras personalizadas: *{len(palavras)}*\n\n{lista}",
            parse_mode=ParseMode.MARKDOWN
        )
...
async def cmd_testemunho(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if context.args:
        ...
        await update.message.reply_text(
            "🌟 *SISTEMA DE TESTEMUNHOS*\n\nPara enviar seu testemunho:\n\n"
            "1️⃣ Use o comando:\n`/testemunho [seu testemunho]`\n\n"
            "2️⃣ Envie uma mensagem de texto direto no privado do bot\n\n"
...
async def cmd_chatid(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user = update.effective_user
    await update.message.reply_text(
        f"📋 *Informações do Chat*\n\n🆔 Chat ID: `{chat.id}`\n📝 Nome: {chat.title or chat.first_name}\n"
        f"🔹 Tipo: {chat.type}\n👤 Seu ID: `{user.id}`\n🔑 Dono registrado: `{OWNER_ID}`",
        parse_mode=ParseMode.MARKDOWN
    )
...
async def cmd_importar(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id != OWNER_ID:
        return
    destino = user_id
    await context.bot.send_message(destino, "⏳ *Iniciando migração JSON → PostgreSQL...*", parse_mode=ParseMode.MARKDOWN)
    try:
        ...
        linhas = "\n".join(f"{'✅' if q > 0 else '⏭️'} {k}: *{q}* registro(s)" for k, q in resultados.items())
        await context.bot.send_message(destino, f"✅ *MIGRAÇÃO CONCLUÍDA!*\n\n{linhas}", parse_mode=ParseMode.MARKDOWN)
    except FileNotFoundError:
        await context.bot.send_message(destino, "⏭️ *Nenhum arquivo JSON antigo encontrado.*", parse_mode=ParseMode.MARKDOWN)
    except Exception as e:
        await context.bot.send_message(destino, f"❌ *Erro:*\n`{str(e)[:200]}`", parse_mode=ParseMode.MARKDOWN)
