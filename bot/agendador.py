...
ENQUETES_SEMANAIS = [
    {"pergunta": "🎵 Qual estilo de música gospel você mais ama?", "opcoes": ["Hinos tradicionais", "Gospel contemporâneo", "Adoração e louvor", "Música instrumental", "Sertanejo gospel"]},
    {"pergunta": "📖 Qual livro da Bíblia você mais gosta de ler?", "opcoes": ["Salmos", "Provérbios", "João", "Romanos", "Apocalipse"]},
    {"pergunta": "🙏 Como você costuma orar?", "opcoes": ["De manhã ao acordar", "À noite antes de dormir", "Durante o dia quando preciso", "Em momentos específicos", "O tempo todo"]},
    {"pergunta": "✝️ O que mais te edifica espiritualmente?", "opcoes": ["Orar", "Ler a Bíblia", "Louvar e adorar", "Comunhão com irmãos", "Jejum e oração"]},
    {"pergunta": "🌅 Qual é o seu versículo favorito?", "opcoes": ["Filipenses 4:13", "João 3:16", "Jeremias 29:11", "Salmos 23:1", "Romanos 8:28"]},
    {"pergunta": "🔥 O que significa avivamento para você?", "opcoes": ["Renovação espiritual pessoal", "Crescimento da Igreja", "Milagres e curas", "Conversão de almas", "Todas as opções acima"]},
    {"pergunta": "⛪ Com que frequência você vai à igreja?", "opcoes": ["Todo domingo", "Várias vezes por semana", "Uma vez por mês", "Em datas especiais", "Assisto online"]},
    {"pergunta": "💪 Como você enfrenta os momentos difíceis?", "opcoes": ["Com oração intensa", "Lendo a Palavra", "Com apoio dos irmãos", "Louvor mesmo na dor", "Todas as anteriores"]},
    {"pergunta": "🌟 Qual promessa bíblica mais te fortalece?", "opcoes": ['"Tudo posso em Cristo"', '"Nunca te deixarei"', '"Sou contigo"', '"Planos de prosperidade"', '"O amor de Deus"']},
    {"pergunta": "🕊️ Como você descreveria sua relação com Deus hoje?", "opcoes": ["Muito próxima e crescendo", "Boa, mas quero mais", "Passando por desafios", "Estou me reconectando", "Em constante busca"]},
]
...
TEMAS_PREGACAO = [
    "A fé que move montanhas", "O amor de Deus é incondicional", "A graça transformadora",
    "Avivamento do Espírito Santo", "A paz que excede todo entendimento",
    "Caminhar com Deus todos os dias", "A esperança que não envergonha",
    "Renovação pela Palavra de Deus", "O poder da oração",
    "Confiança no Senhor em tempos difíceis", "A cura que vem do Alto",
    "Vitória em Cristo Jesus", "O Espírito Santo como Consolador",
    "Gratidão que transforma", "A Palavra que não volta vazia",
]
...
async def postar_midia_no_canal(context: ContextTypes.DEFAULT_TYPE) -> bool:
    try:
        ...
        if tipo == "video":
            await context.bot.send_video(CHANNEL_ID, video=midia["file_id"], caption=caption_base, parse_mode=ParseMode.MARKDOWN, supports_streaming=True)
        else:
            await context.bot.send_photo(CHANNEL_ID, photo=midia["file_id"], caption=caption_base, parse_mode=ParseMode.MARKDOWN)
...
async def postar_devocional_canal(context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        await context.bot.send_message(CHANNEL_ID, text=texto, parse_mode=ParseMode.MARKDOWN)
...
async def postar_mensagem_biblica_grupo(context: ContextTypes.DEFAULT_TYPE):
    try:
        buf, referencia, texto = gerar_imagem_versiculo()
        caption = f'📖 *{referencia}*\n\n_"{texto}"_\n\n🕊️ *Avivamento AD*\n\n[📤 Compartilhe]({LINK_GRUPO})'
        await context.bot.send_photo(GROUP_ID, photo=buf, caption=caption, parse_mode=ParseMode.MARKDOWN)
...
async def postar_oracao_grupo(context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        await context.bot.send_message(GROUP_ID, oracao, parse_mode=ParseMode.MARKDOWN)
...
async def postar_engajamento_grupo(context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        await context.bot.send_message(GROUP_ID, pergunta, parse_mode=ParseMode.MARKDOWN)
...
async def verificar_aniversarios(context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        mensagens_bolo = [
            ...
            "🎁🔥 Que este ano seja repleto das bênçãos do Senhor!",
        ...
        for uid, nome, dia, mes in aniversariantes:
            ...
            texto_grupo = (
                ...
                f'_"O Senhor te abençoe e te guarde."_ — Números 6:24\n\n'
            ...
            try:
                texto_privado = (
                    ...
                    f"A família Avivamento AD te deseja um dia abençoado! 🔥🎉\n\n"
                    f'_"Porque sou eu que conheço os planos que tenho para vocês..."_ — Jeremias 29:11'
                )
...
async def postar_testemunho_canal(context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        await context.bot.send_message(
            GROUP_ID,
            f'🌟 *TESTEMUNHO DA SEMANA no canal!*\nAcesse o canal para ler!\n[👉 Ver no canal]({LINK_CANAL})',
...
async def postar_enquete_grupo(context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        await context.bot.send_poll(
            chat_id=GROUP_ID, question=enquete["pergunta"], options=enquete["opcoes"],
            is_anonymous=False, allows_multiple_answers=False,
        )
        await context.bot.send_message(
            GROUP_ID,
            f'🗳️ *ENQUETE DA SEMANA!*\n\nParticipe e compartilhe sua opinião! 🙏\n\n'
            f'[📤 Convide alguém]({LINK_GRUPO})',
            parse_mode=ParseMode.MARKDOWN
        )
